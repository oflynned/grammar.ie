...
<LogicSection title="What is arna?">
    The forms `arna` and `arnár` are only used in connection with the verbal noun (meaning "after"), serving as
    replacements for the archaic `iar n-a` and `iar n-ár`. Otherwise, `ar a` and `ar ár` are used.

    `Arna` also appears in the specific phrase `an lá arna mhárach` ("the following day" - tomorrow).
...
<MutationCard title="General State (Set Phrases)" before="Ar cíos" after="Ar cíos" rule="No mutation occurs in fixed or general adverbial states (e.g., for rent, on foot)." />

<MutationCard title="Qualified Indefinite Nouns" before="Ar cíos mór" after="Ar chíos mór" rule="Even in set phrases, if the noun is qualified by an adjective, lenition is usually triggered." />

<MutationCard title="Indefinite Nouns (No Article)" before="Ar bord" after="Ar bhord" rule="Causes lenition on the following noun when no article is present." />

<MutationCard title="Definite Nouns (With Article)" before="Ar an mbord" after="Ar an mbord" rule="Causes eclipsis when followed by the singular article." />
